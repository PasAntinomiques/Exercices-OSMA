	...
	private class optimisationInitialisation extends OneShotBehaviour {
		...
		public void action() {
			...
			case "tabou": {
				...
				taboulet2 k=new taboulet2();
				instantSol = k.tabou(cities);
...
//					Ici on exécute le main de tabou en partant d'une solution admissible
					taboulet2 k=new taboulet2();
					instantSol = k.tabou(cities, instantSol);
					cout = distance(instantSol);
					break;
				}
				case"recuit":{
			...
			while (nbMessages < nombreDeCorrespondants) {
				...
				if (msg != null) {
					try {
						...
						// System.out.println("Agent "+myAgent.getLocalName()+" : Receiving results from
